...
class ShouldBeEquivalentBacktickTest {
    ...
    fun `pass should be equivalent to as includedProperties are equal`() {
        ...
        // assert
        team1 `should be equivalent to` (team2 to {
            options: EquivalencyAssertionOptions -> options.including(ShouldBeEquivalentTo.Team::persons)
        })
    }
    ...
    fun `pass should not be equivalent to as includedProperties are different`() {
        ...
        // assert
        team1 `should not be equivalent to` Pair<ShouldBeEquivalentTo.Team, ((EquivalencyAssertionOptions) -> EquivalencyAssertionOptions)?> (team2, {
            options: EquivalencyAssertionOptions -> options.including(ShouldBeEquivalentTo.Team::persons)
        })
    }
}